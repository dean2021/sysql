...
// 通过pid获取网络连接信息
func ConnectionsInodeWithPid(kind string, pid int32, nsInodeMap map[string]bool) ([]Connection, error) {
	tmap, ok := netConnectionKindMap[kind]
	if !ok {
		return nil, fmt.Errorf("invalid kind, %s", kind)
	}
	...
	for _, t := range tmap {
		...
		switch t.family {
		case syscall.AF_INET, syscall.AF_INET6:
			ls, err = parserInet(path, t)
		}
		if err != nil {
			return nil, err
		}
		for _, c := range ls {
			if _, ok := nsInodeMap[c.inode]; !ok {
				continue
			}
			// Build TCP key to id the connection uniquely
			// socket type, src ip, src port, dst ip, dst port and state should be enough
...
var SockStatsRegexp = regexp.MustCompile(`sockets: used ([0-9]+) TCP: inuse ([0-9]+) orphan ([0-9]+) tw ([0-9]+) alloc ([0-9]+) mem ([0-9]+) UDP: inuse ([0-9]+) mem ([0-9]+) UDPLITE: inuse ([0-9]+) RAW: inuse ([0-9]+) FRAG: inuse ([0-9]+) memory ([0-9]+)`)

func ParserSockStat() (*SockStats, error) {
	file := common.HostProc("net", "sockstat")
	contents, err := ioutil.ReadFile(file)
	if err != nil {
		return nil, err
	}
	all := bytes.ReplaceAll(contents, []byte("\n"), []byte(" "))
	subMatch := SockStatsRegexp.FindSubmatch(all)
	var sockStats = &SockStats{}
	for k, v := range subMatch {
		switch k {
		case 1:
			sockStats.SocketsUsed, _ = strconv.Atoi(string(v))
		case 2:
			sockStats.TcpInUse, _ = strconv.Atoi(string(v))
		case 3:
			sockStats.TcpOrphan, _ = strconv.Atoi(string(v))
		case 4:
			sockStats.TcpTw, _ = strconv.Atoi(string(v))
		case 5:
			sockStats.TcpAlloc, _ = strconv.Atoi(string(v))
		case 6:
			sockStats.TcpMem, _ = strconv.Atoi(string(v))
		case 7:
			sockStats.UdpInuse, _ = strconv.Atoi(string(v))
		case 8:
			sockStats.UdpMem, _ = strconv.Atoi(string(v))
		case 9:
			sockStats.UdpLiteInuse, _ = strconv.Atoi(string(v))
		case 10:
			sockStats.RawInuse, _ = strconv.Atoi(string(v))
		case 11:
			sockStats.FragInuse, _ = strconv.Atoi(string(v))
		case 12:
			sockStats.FragMem, _ = strconv.Atoi(string(v))
		}
	}
