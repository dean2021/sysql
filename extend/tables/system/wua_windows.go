...
func (c *IUpdateCollection) extractPkg(item int) (*WUAPackage, error) {
	...
	var severity string
	msrcSeverity, err := updt.GetProperty("MsrcSeverity")
	if err != nil {
		return nil, fmt.Errorf(`updt.GetProperty("MsrcSeverity"): %v`, err)
	}
	if msrcSeverity.ToString() == "" {
		severity = "Undefined"
	} else {
		severity = msrcSeverity.ToString()
	}
...
// WUAUpdates queries the Windows Update Agent API searcher with the provided sysql.
func WUAUpdates(query string) ([]WUAPackage, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("error calling GetWUAUpdateCollection with sysql %q: %v", query, err)
	}
	defer updts.Release()
...
// GetWUAUpdateCollection queries the Windows Update Agent API searcher with the provided sysql
